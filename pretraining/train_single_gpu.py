...
def get_batch(dataloader, device, global_step):
    ...
    if dataset is not None and hasattr(dataset, "set_global_step"):
...
def save(model, ema_model, optimizer, scheduler, global_step, masked_epoch, causal_epoch, args):
    if is_main_process():
        ...
        torch.save(ema_model.state_dict(), str(args.output_path).replace(".bin", "_ema.bin"))
        torch.save(
            {
            ...
            str(args.output_path).replace(".bin", "_state_dict.bin")
        )
        ...
        wandb.log(
            {
                ...
                "stats/local_batch_size": args.local_batch_size,
                "stats/accumulate_steps": accumulate_steps,
